...
var phoneContact = [PhoneContact]()
    ...
    func getContacts() {
        ...
        if CNContactStore.authorizationStatus(for: .contacts) == .notDetermined {
            self.CNcontactStore.requestAccess(for: .contacts, completionHandler: { (authorized: Bool, error: Error?) -> Void in
                if authorized {
        ...
        func retrieveContactsWithStore() {
            do {
                let groups = try self.CNcontactStore.groups(matching: nil)
                ...
                let contacts = try self.CNcontactStore.unifiedContacts(matching: predicate, keysToFetch: keysToFetch as! [CNKeyDescriptor])
    ...
    private func readPhoneContact()
    {
        ...
        do {
            try self.CNcontactStore.enumerateContacts(with: request) {
                (contact, stop) in
